...
def forward(rundir_or_config_path, event_names):
    ...
    if op.isdir(rundir_or_config_path):
        ...
        problem, xs, misfits, _ = load_problem_info_and_data(rundir, subset="harvest")
        ...
        for event_name in event_names:
            ...
            xref = problem.preconstrain(problem.pack(problem.base_source))
...
def harvest(rundir, problem=None, nbest=10, force=False, weed=0):
    ...
    if problem is None:
        problem, xs, misfits, bootstrap_misfits, _ = load_problem_info_and_data(
            rundir, nchains=nchains
        )
    else:
        xs, misfits, bootstrap_misfits, _ = load_problem_data(
            rundir, problem, nchains=nchains
        )
    ...
    if weed != 3:
        ...
        if weed:
            ...
            ibests_list = [
                ibests_
                for (ibootstrap, ibests_) in enumerate(ibests_list)
...
def cluster(rundir, clustering, metric):
    ...
    for i in range(labels.size):
        if labels[i] == -1:
            logging.info("Number of unclustered events: %5i" % ns[i])
        else:
            logging.info("Number of events in cluster %i: %5i" % (labels[i], ns[i]))
...
def check(
            ...
            if show_waveforms:
                ...
                for target in problem.targets:
                    ...
                    for source in sources:
                        ...
                        times.extend((tmin_fit - tfade * 2.0, tmax_fit + tfade * 2.0))
                ...
                for (tfade, tobs_shift, tcut), target in zip(tdata, problem.targets):
                    ...
                    try:
                        trs_projected, trs_restituted, trs_raw, _ = ds.get_waveform(
                            target.codes,
                    ...
                    for trx in trs_projected + trs_restituted + trs_raw:
                        ...
                        trx.set_codes(
                            network="", station=target.string_id(), location=""
                        )
                    ...
                    markers.append(
                        pmarker.Marker(
                            ...
                            tmin=tcut[0] - tobs_shift,
                            tmax=tcut[1] - tobs_shift,
                            kind=1,
                        )
                    )
                ...
                for itarget, target in enumerate(problem.targets):
                    ...
                    logger.info("%-40s %s" % ((target.string_id() + ":", sok)))

        except GrondError as e:
            logger.error('Event %i, "%s": %s' % (iselected, name, str(e)))
    ...
    if erroneous:
        raise GrondError("Check failed for events: %s" % ", ".join(erroneous))


g_state = {}


def go(environment, force=False, preserve=False, nparallel=1, status="state"):
    g_data = (environment, force, preserve, status, nparallel)
    g_state[id(g_data)] = g_data

    nselected = environment.nselected
    for x in parimap.parimap(
        process_event, range(nselected), [id(g_data)] * nselected, nprocs=nparallel
    ):
        pass


def process_event(iselected, g_data_id):
    environment, force, preserve, status, nparallel = g_state[g_data_id]
    ...
    rundir = expand_template(config.rundir_template, dict(problem_name=problem.name))
    ...
    logger.info(
        "Processing problem %i / %i: %s" % (iselected + 1, nselected, problem.name)
    )
    ...
    try:
        if xs_inject is not None:
            ...
            if not isinstance(optimiser, highscore.HighScoreOptimiser):
                raise GrondError("Optimiser does not support injections.")
    ...
    tstop = time.time()
    logger.info(
        "Stop %i / %i (%g min)" % (iselected + 1, nselected, (tstop - tstart) / 60.0)
    )

    logger.info('Done with problem "%s", rundir is "%s".' % (problem.name, rundir))
...
class ParameterStats(Object):
    ...
    def __mul__(self, other):
        if type(other) == float:
            multi = []
            for i in self.T.propnames[1:]:
                multi.append(getattr(self, i) * other)
            multi = ["Test"] + multi
            return ParameterStats(*multi)
        else:
            multi = []
            for i in self.T.propnames[1:]:
                multi.append(getattr(self, i) * getattr(other, i))
            multi = ["Test"] + multi
            return ParameterStats(*multi)

    def calc_mag(self):
        if "moment" in self.name:
            multi = []
            for i in self.T.propnames[1:]:
                multi.append(num.round(2 / 3 * (num.log10(getattr(self, i)) - 9.1), 2))
            multi = ["Mww"] + multi
...
def make_stats(problem, xs, misfits, pnames=None):
    ...
    for pname in pnames:
        ...
        mi, p2_5, p5, p16, median, p84, p95, p97_5, ma = map(
            float,
            num.percentile(vs, [0.0, 2.5, 5.0, 16.0, 50.0, 84.0, 95.0, 97.5, 100.0]),
        )
        ...
        s = ParameterStats(
            pname, mean, std, best, mi, p2_5, p5, p16, median, p84, p95, p97_5, ma
        )
        logger.warning(s)
        rs.parameter_stats_list.append(s)

    def get_param(rs, name):
        for i in rs.parameter_stats_list:
            if name == i.name:
                return i
            else:
                pass

    for ssource in problem.base_source.subsources:
        name = ssource.name
        moment = (
            get_param(rs, name + ".slip")
            * get_param(rs, name + ".length")
            * get_param(rs, name + ".width")
            * 32.0e9
        )
        moment.name = name + ".moment"
        Mww = moment.calc_mag()
        area = get_param(rs, name + ".length") * get_param(rs, name + ".width")
        area.name = name + ".area"
        rs.parameter_stats_list.append(moment)
        rs.parameter_stats_list.append(Mww)
        rs.parameter_stats_list.append(area)
...
def export(what, rundirs, type=None, pnames=None, filename=None):
    ...
    if what == "stats" and type is not None:
        raise GrondError(
            "Invalid argument combination: what=%s, type=%s" % (repr(what), repr(type))
        )

    if what != "stats" and shortform:
        raise GrondError(
            "Invalid argument combination: what=%s, pnames=%s"
