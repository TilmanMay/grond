...
class JointparPlot(PlotConfig):
    ...
    name = "jointpar"
    size_cm = Tuple.T(2, Float.T(), default=(20.0, 20.0))
    ...
    def make(self, environ):
        ...
        sref = "Dark gray boxes mark reference solution." if self.show_reference else ""
...
parameter bounds and shows the model space of the optimsation. %s"""
            % sref,
        )
    ...
    def draw_figures(self, history, optimiser):
        ...
        if color_parameter == "dist":
            ...
            raise meta.GrondError("Invalid color_parameter: %s" % color_parameter)
        ...
        if nselected < 2:
            logger.warn(
                "Cannot draw joinpar figures with less than two " "parameters selected."
            )
            return []
        ...
        for iselected in range(nselected):
            ...
            for jselected in range(iselected):
                ...
                axes.axvline(0.0, color=mpl_color("aluminium3"), lw=0.5)
                axes.axhline(0.0, color=mpl_color("aluminium3"), lw=0.5)
                ...
                if ix == 0 or jselected + 1 == iselected:
                    for xpos, xoff, x in [(0.0, 10.0, xmin), (1.0, -10.0, xmax)]:
                        axes.annotate(
                ...
                if iy == nsubplots - 1 or jselected + 1 == iselected:
                    for ypos, yoff, y in [(0.0, 10.0, ymin), (1.0, -10.0, ymax)]:
                        axes.annotate(
                ...
                if iselected == nselected - 1 or ix == nsubplots - 1:
                    try:
                        grp = xpar._name.split(".")[0].split("_")[1] + "."
                    except:
                        grp = ""
                    logger.warning(grp)
                    axes.annotate(
                        grp + xpar.get_label(with_unit=False),
                        xy=(0.5, -0.05),
                        xycoords="axes fraction",
                ...
                if iy == 0:
                    try:
                        grp = ypar._name.split(".")[0].split("_")[1] + "."
                    except:
                        grp = ""
                    axes.annotate(
                        grp + ypar.get_label(with_unit=False),
                        xy=(-0.05, 0.5),
                        xycoords="axes fraction",
                        verticalalignment="top",
                        horizontalalignment="right",
        ...
        for figs_row in figs:
            figs_flat.extend(item_fig for item_fig in figs_row if item_fig is not None)

        return figs_flat


class CovarPlot(PlotConfig):
    """
    Source problem parameter's tradeoff plots.
    """
    ...
    def make(self, environ):
        ...
        cm.create_group_mpl(
            ...
            feather_icon="crosshair",
            description="""
Covaricance plots for Jointpar...hopefully ¯\_(ツ)_/¯%s"""
            % sref,
        )
    ...
    def draw_figures(self, history, optimiser):
        ...
        if nselected < 2:
            logger.warn(
                "Cannot draw covariance figures with less than two "
                "parameters selected."
            )
            return []
        ...
        for iselected in range(nselected):
            ...
            for jselected in range(iselected):
                ...
                covars.append(num.corrcoef((fx, fy))[0, 1])

        cmap = cm.ScalarMappable(
            norm=mcolors.Normalize(num.min(covars), vmax=num.max(covars)),
        ...
        for iselected in range(nselected):
            ...
            for jselected in range(iselected):
                ...
                covar = num.corrcoef((fx, fy))[0, 1]

                col = cmap.to_rgba(covar)
                axes.set_facecolor(col)
                axes.set_xticks([])
                axes.set_yticks([])
...
class HistogramPlot(PlotConfig):
    ...
    exclude = List.T(String.T())
    include = List.T(String.T())
    method = StringChoice.T(choices=["gaussian_kde", "histogram"], default="histogram")
    show_reference = Bool.T(default=True)
    ...
    def draw_figures(self, history):
        ...
        for iselected in range(nselected):
            ...
            labelpos = mpl_margins(
                fig, nw=1, nh=1, w=7.0, bottom=5.0, top=1, units=fontsize
            )
            ...
            if method == "gaussian_kde":
                ...
                pps, edges = num.histogram(
                    vs, bins=num.linspace(vmin, vmax, num=40), density=True
                )
            ...
            axes.axvspan(
                par.scaled(pstats.percentile2_5),
                par.scaled(pstats.percentile97_5),
                color=stats_color,
                alpha=0.1,
            )

            axes.axvline(par.scaled(pstats.median), color=stats_color3, alpha=0.5)
            axes.axvline(par.scaled(pstats.mean), color=stats_color3, ls=":", alpha=0.5)
            ...
            if self.show_reference:
                try:
                    ...
                    axes.axvline(
                        par.scaled(problem.extract(xref, ipar - 1)), color=ref_color
                    )
...
class MTDecompositionPlot(PlotConfig):
    ...
    size_cm = Tuple.T(2, Float.T(), default=(15.0, 5.0))
...
"""
    ...
    def draw_figures(self, history):
        ...
        best_source = stats.get_best_source(problem, history.models, history.misfits)

        ref_source = problem.base_source

        nlines_max = int(round(self.size_cm[1] / 5.0 * 4.0 - 1.0))

        if self.cluster_attribute:
            cluster_sources = history.mean_sources_by_cluster(self.cluster_attribute)
        else:
            cluster_sources = []
        ...
        lines = []
        lines.append(("Ensemble best", get_deco(best_source), mpl_color("aluminium5")))

        lines.append(("Ensemble mean", get_deco(mean_source), mpl_color("aluminium5")))
        ...
        for icluster, perc, source in cluster_sources:
            if len(lines) < nlines_max - int(self.show_reference):
                ...
                logger.warn(
                    ...
                    % (
                        icluster,
                        (3 + len(cluster_sources) + int(self.show_reference)) * 5 / 4.0,
                    )
                )

        if self.show_reference:
            lines.append(("Reference", get_deco(ref_source), mpl_color("aluminium3")))
        ...
        for i, (label, deco, color_t) in enumerate(lines):
            ...
            for xpos, mt_part, ratio, ops in [
                (0.0, m_full, ratio_full, "-"),
                (2.0, m_iso, ratio_iso, "="),
                (4.0, m_devi, ratio_devi, "="),
                (6.0, m_clvd, ratio_clvd, "+"),
                (8.0, m_dc, ratio_dc, None),
            ]:
                if ratio > 1e-4:
                    try:
...
class MTLocationPlot(SectionPlotConfig):
    ...
    name = "location_mt"
    beachball_type = StringChoice.T(choices=["full", "deviatoric", "dc"], default="dc")
    ...
    def draw_figures(self, history):
        ...
        for axes, xparname, yparname in [
            ...
            for ix, x in enumerate(models):
                ...
                alpha = (iorder[ix] - num.min(iorder)) / float(
                    num.max(iorder) - num.min(iorder)
                )
...
class MTFuzzyPlot(PlotConfig):
    ...
    name = "mt_fuzzy"
    size_cm = Tuple.T(2, Float.T(), default=(10.0, 10.0))
    cluster_attribute = meta.StringID.T(
        optional=True, help="name of attribute to use as cluster IDs"
    )
...
"""
    ...
    def draw_figures(self, history):
        ...
        by_cluster = history.imodels_by_cluster(self.cluster_attribute)
        ...
        for icluster, percentage, imodels in by_cluster:
            ...
            beachball.plot_fuzzy_beachball_mpl_pixmap(
                ...
                best_mt,
                beachball_type="full",
                size=8.0 * math.sqrt(percentage / 100.0),
                position=(5.0, 5.0),
                color_t=color,
                edgecolor="black",
...
class HudsonPlot(PlotConfig):
    ...
    name = "hudson"
    size_cm = Tuple.T(2, Float.T(), default=(17.5, 17.5 * (3.0 / 4.0)))
    beachball_type = StringChoice.T(choices=["full", "deviatoric", "dc"], default="dc")
    show_reference = Bool.T(default=True)
