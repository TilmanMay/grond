...
class STFType(StringChoice):
    ...
    cls = {"HalfSinusoidSTF": gf.HalfSinusoidSTF, "ResonatorSTF": gf.ResonatorSTF}
...
class CMTProblem(Problem):
    ...
    problem_parameters_stf = {
        "HalfSinusoidSTF": [Parameter("duration", "s", label="Duration")],
        "ResonatorSTF": [
    ...
    dependants = [
        Parameter("strike1", "\u00b0", label="Strike 1"),
        Parameter("dip1", "\u00b0", label="Dip 1"),
        Parameter("rake1", "\u00b0", label="Rake 1"),
        Parameter("strike2", "\u00b0", label="Strike 2"),
        Parameter("dip2", "\u00b0", label="Dip 2"),
        Parameter("rake2", "\u00b0", label="Rake 2"),
    ...
    def __init__(self, **kwargs):
        ...
        self.problem_parameters = (
            self.problem_parameters + self.problem_parameters_stf[self.stf_type]
        )
    ...
    def get_source(self, x):
        ...
        rm6 = num.array(
            [d.rmnn, d.rmee, d.rmdd, d.rmne, d.rmnd, d.rmed], dtype=num.float64
        )
    ...
    def preconstrain(self, x):
        ...
        if self.mt_type == "deviatoric":
            ...
            m_iso = num.diag([trace_m / 3.0, trace_m / 3.0, trace_m / 3.0])
            m9 -= m_iso
        ...
        m0_unscaled = math.sqrt(num.sum(as_arr(m9) ** 2)) / math.sqrt(2.0)
...
class MultiCMTProblem(Problem):
    ...
    def __init__(self, **kwargs):
        ...
        for source in self.base_source.subsources:
            for bparameter in (
                self.problem_parameters_single
    ...
    def pack_stf(self, stf):
        return [stf[p.name] for p in self.problem_parameters_stf_single[self.stf_type]]
