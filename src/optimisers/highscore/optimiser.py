...
from grond.optimisers.base import (
    Optimiser,
    OptimiserConfig,
    BadProblem,
    OptimiserStatus,
...
def excentricity_compensated_probabilities(xs, sbx, factor):
    ...
    scale[inonflat] = 1.0 / (sbx[inonflat] * (factor if factor != 0.0 else 1.0))
    distances_sqr_all = num.sum(
        (
            (xs[num.newaxis, :, :] - xs[:, num.newaxis, :])
            * scale[num.newaxis, num.newaxis, :]
        )
        ** 2,
        axis=2,
    )
...
def excentricity_compensated_choice(xs, sbx, factor, rstate):
    probabilities = excentricity_compensated_probabilities(xs, sbx, factor)
...
class SamplerPhase(Object):
    niterations = Int.T(help="Number of iteration for this phase.")
    ntries_preconstrain_limit = Int.T(
        default=1000, help="Tries to find a valid preconstrained sample."
    )
    seed = Int.T(optional=True, help="Random state seed.")
...
class InjectionSamplerPhase(SamplerPhase):
    xs_inject = Array.T(
        dtype=num.float64, shape=(None, None), help="Array with the reference model."
    )
...
class DirectedSamplerPhase(SamplerPhase):
    scatter_scale = Float.T(
        optional=True, help="Scales search radius around the current `highscore` models"
    )
    scatter_scale_begin = Float.T(
        optional=True, help="Scaling factor at beginning of the phase."
    )
    scatter_scale_end = Float.T(
        optional=True, help="Scaling factor at the end of the directed phase."
    )
    ...
    sampler_distribution = SamplerDistributionChoice.T(
        default="normal", help="Distribution new models are drawn from."
    )
    ...
    def get_raw_sample(self, problem, iiter, chains):
        ...
        if self.starting_point == "excentricity_compensated":
            ...
            assert False, "invalid starting_point choice: %s" % (self.starting_point)

        ntries_sample = 0
        if self.sampler_distribution == "normal":
            x = num.zeros(npar, dtype=num.float64)
            ...
            for ipar in range(npar):
                ...
                while True:
                    if sx[ipar] > 0.0:
                        v = rstate.normal(xchoice[ipar], factor * sx[ipar])
                    else:
                        v = xchoice[ipar]

                    if xbounds[ipar, 0] <= v and v <= xbounds[ipar, 1]:
                        break
                    ...
                    if ntries > self.ntries_sample_limit:
                        logger.warning(
                            ...
                            "- drawing from uniform instead." % pnames[ipar]
                        )
            ...
            while True:
                ...
                if ntries_sample > self.ntries_sample_limit:
                    logger.warning(
                        ...
                        % ", ".join("%s:%i" % xx for xx in zip(pnames, ok_mask_sum))
                    )
                    xbounds = problem.get_parameter_bounds()
...
def make_bayesian_weights(nbootstrap, nmisfits, type="bayesian", rstate=None):
    ...
    for ibootstrap in range(nbootstrap):
        if type == "classic":
            ...
            ws[ibootstrap, :] = num.histogram(ii, nmisfits, (-0.5, nmisfits - 0.5))[0]
        elif type == "bayesian":
            f = rstate.uniform(0.0, 1.0, size=nmisfits + 1)
            f[0] = 0.0
            f[-1] = 1.0
            f = num.sort(f)
            g = f[1:] - f[:-1]
...
class Chains(object):
    def __init__(self, problem, history, nchains, nlinks_cap):
        ...
        self.chains_m = num.zeros((self.nchains, nlinks_cap), dtype=num.float64)
        self.chains_i = num.zeros((self.nchains, nlinks_cap), dtype=num.int)
        self.nlinks = 0
        self.nread = 0

        self.accept_sum = num.zeros(self.nchains, dtype=num.int)
        self._acceptance_history = num.zeros((self.nchains, 1024), dtype=num.bool)
    ...
    def goto(self, n=None):
        ...
        while self.nread < n:
            ...
            if self.nlinks == self.nlinks_cap:
                accept = (chains_i[:, self.nlinks_cap - 1] != nread).astype(num.bool)
                self.nlinks -= 1
            else:
    ...
    def _append_acceptance(self, acceptance):
        if self.nread >= self._acceptance_history.shape[1]:
            ...
            new_buf[:, : self._acceptance_history.shape[1]] = self._acceptance_history
...
class HighScoreOptimiser(Optimiser):
    ...
    def get_rstate_bootstrap(self):
        if self._rstate_bootstrap is None:
            self._rstate_bootstrap = num.random.RandomState(self.bootstrap_seed)
    ...
    def init_bootstrap_weights(self, problem):
        ...
        nmisfits_w = sum(t.nmisfits for t in problem.targets if t.can_bootstrap_weights)

        ws = make_bayesian_weights(
            self.nbootstrap, nmisfits=nmisfits_w, rstate=self.get_rstate_bootstrap()
        )
        ...
        for t in problem.targets:
            if t.can_bootstrap_weights:
                ...
                t.set_bootstrap_weights(num.ones((self.nbootstrap, t.nmisfits)))
    ...
    def get_bootstrap_weights(self, problem):
        if self._bootstrap_weights is None:
            ...
            bootstrap_weights = num.hstack(
                [t.get_bootstrap_weights() for t in problem.targets]
            )

            self._bootstrap_weights = num.vstack(
                (num.ones((1, problem.nmisfits)), bootstrap_weights)
            )
    ...
    def get_bootstrap_residuals(self, problem):
        if self._bootstrap_residuals is None:
            ...
            bootstrap_residuals = num.hstack(
                [t.get_bootstrap_residuals() for t in problem.targets]
            )

            self._bootstrap_residuals = num.vstack(
                (num.zeros((1, problem.nmisfits)), bootstrap_residuals)
            )
    ...
    def chains(self, problem, history):
        nlinks_cap = int(round(self.chain_length_factor * problem.nparameters + 1))

        return Chains(problem, history, nchains=self.nchains, nlinks_cap=nlinks_cap)
    ...
    def optimise(self, problem, rundir=None):
        ...
        history = ModelHistory(problem, nchains=self.nchains, path=rundir, mode="w")
        ...
        for iiter in range(niter):
            ...
            if isbad_mask is not None and num.any(isbad_mask != isbad_mask_new):
                errmess = [
                    ...
                    " at iteration %i" % (problem.name, iiter)
                ]
                ...
                for target, isbad_new, isbad in zip(
                    ...
                    if isbad_new != isbad:
                        errmess.append(
                            "  %s, %s -> %s" % (target.string_id(), isbad, isbad_new)
                        )
            ...
            history.append(
                sample.model, misfits, bootstrap_misfits, sample.pack_context()
            )
    ...
    def get_status(self, history):
        ...
        acceptance_latest = chains.acceptance_history[
            :, -min(chains.acceptance_history.shape[1], self.ACCEPTANCE_AVG_LEN) :
        ]  # noqa
        ...
        def spark_plot(data, bins):
            ...
            if hist_max == 0.0:
                hist_max = 1.0
            hist = hist / hist_max
            vec = num.digitize(hist, num.linspace(0.0, 1.0, len(self.SPARKS)))
        ...
        return OptimiserStatus(
            ...
            column_data=OrderedDict(
                zip(
                    ["BS mean", "BS std", "Glob mean", "Glob std", "Glob best"],
                    [bs_mean, bs_std, glob_mean, glob_std, glob_best],
                )
            ),
            extra_header="Optimiser phase: {phase}, exploring {nchains} BS chains\n"  # noqa  # noqa
            "Global chain misfit distribution: \u2080{mf_dist}\xb9\n"
            "Acceptance rate distribution:     \u2080{acceptance}"
            "\u2081\u2080\u2080\ufe6a (Median {acceptance_med:.1f}%)".format(
                phase=phase.__class__.__name__,
                nchains=chains.nchains,
                mf_dist=spark_plot(glob_misfits, num.linspace(0.0, 1.0, 25)),
                acceptance=spark_plot(acceptance_avg, num.linspace(0.0, 1.0, 25)),
                acceptance_med=num.median(acceptance_avg) * 100.0,
            ),
        )

    def get_movie_maker(self, problem, history, xpar_name, ypar_name, movie_filename):
        from . import plot
...
class HighScoreOptimiserConfig(OptimiserConfig):
    sampler_phases = List.T(
        SamplerPhase.T(),
        default=[
            UniformSamplerPhase(niterations=1000),
...
def load_optimiser_history(dirname, problem):
    ...
    with open(fn, "r") as f:
        ...
        data1 = num.fromfile(
            f, dtype="<i1", count=nmodels * (problem.nbootstrap + 1)
        ).astype(num.bool)
    ...
    with open(fn, "r") as f:
        data2 = num.fromfile(f, dtype="<i8", count=nmodels * 2).astype(num.int64)
