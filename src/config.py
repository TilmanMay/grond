...
class EngineConfig(HasPaths):
    gf_stores_from_pyrocko_config = Bool.T(
        default=True, help="Load the GF stores from ~/.pyrocko/config"
    )
    gf_store_superdirs = List.T(
        Path.T(), help="List of path hosting collection of Green's function stores."
    )
    gf_store_dirs = List.T(Path.T(), help="List of Green's function stores")
...
class Config(HasPaths):
    rundir_template = Path.T(
        help="Rundir for the optimisation, supports templating" " (eg. ${event_name})"
    )
    dataset_config = DatasetConfig.T(help="Dataset configuration object")
    target_groups = List.T(TargetGroup.T(), help="List of ``TargetGroup``s")
    problem_config = ProblemConfig.T(help="Problem config")
    analyser_configs = List.T(
        AnalyserConfig.T(),
        default=[TargetBalancingAnalyserConfig.D()],
        help="List of problem analysers",
    )
    optimiser_config = OptimiserConfig.T(help="The optimisers configuration")
    engine_config = EngineConfig.T(
        default=EngineConfig.D(), help=":class:`pyrocko.gf.LocalEngine` configuration"
    )
    event_names = List.T(
        gf.StringID.T(),
    ...
    event_names_exclude = List.T(gf.StringID.T(), help="Event names to be excluded")
    ...
    def get_targets(self, event):
        ...
        for igroup, target_group in enumerate(self.target_groups):
            targets.extend(target_group.get_targets(ds, event, "target.%i" % igroup))
        return targets
    ...
    def get_problem(self, event):
        ...
        problem = self.problem_config.get_problem(event, self.target_groups, targets)
...
def diff_configs(path1, path2):
    ...
    result = list(
        difflib.unified_diff(s1.splitlines(1), s2.splitlines(1), "left", "right")
    )
...
def parse_yname(yname):
    ...
    rint = r"-?[0-9]+"
    m = re.match(
        r"^(%s)(\[((%s)?(:)(%s)?|(%s))\])?$" % (ident, rint, rint, rint), yname
    )
