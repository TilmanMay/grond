...
class StationDistributionPlot(PlotConfig):
    ...
    size_cm = Tuple.T(
        2, Float.T(), default=(16.0, 13.0), help="width and length of the figure in cm"
    )
    font_size = Float.T(default=10, help="font size of all text, except station labels")
    font_size_labels = Float.T(default=6, help="font size of station labels")
    ...
    def plot_station_distribution(
        ...
        if colors is None:
            scolors = [scatter_default["c"] if s else invalid_color for s in valid]
        else:
            if cnorm is None:
        ...
        ws, exp, fmt = make_scale(0.0, weights_ref)
        ws = ws[1:]
        ...
        if len(labels) < 30:  # TODO: remove after impl. of collision detection
            if labels is not None:
                for ilbl, label in enumerate(labels):
                    ax.annotate(
                        label,
                        (azimuths[ilbl] * d2r, distances[ilbl]),
                        **annotate_default
                    )
