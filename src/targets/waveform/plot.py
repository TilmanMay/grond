...
def plot_dtrace(axes, tr, space, mi, ma, **kwargs):
    t = tr.get_xdata()
    y = tr.get_ydata()
    y2 = (num.concatenate((y, num.zeros(y.size))) - mi) / (ma - mi) * space - (
        1.0 + space
    )
    t2 = num.concatenate((t, t[::-1]))
    axes.fill(t2, y2, clip_on=False, **kwargs)
...
class CheckWaveformsPlot(PlotConfig):
    ...
    size_cm = Tuple.T(
        2, Float.T(), default=(9.0, 7.5), help="width and length of the figure in cm"
    )
    n_random_synthetics = Int.T(default=10, help="Number of Synthetics to generate")
...
""",
    ...
    def draw_figures(self, sources, targets, results_list):
        ...
        for itarget, target, results in zip(range(len(targets)), targets, results_list):
    ...
    def draw_figure(self, sources, target, results):
        ...
        for result in results:
            if isinstance(result, WaveformMisfitResult):
                yabsmaxs.append(num.max(num.abs(result.filtered_obs.get_ydata())))
        ...
        labelpos = plot.mpl_margins(fig, nw=1, nh=1, w=1.0, h=5.0, units=fontsize)
        ...
        for source, result in zip(sources, results):
            ...
            axes.plot(t - t0, ydata * 0.40 + 3.5, color="black", lw=1.0)
...
class FitsWaveformEnsemblePlot(PlotConfig):
    ...
    size_cm = Tuple.T(
        2, Float.T(), default=(9.0, 5.0), help="width and length of the figure in cm"
    )
    ...
    color_parameter = String.T(
        default="misfit", help="Choice of value to color, options: dist and misfit"
    )
    ...
    def draw_figures(self, ds, history):
        ...
        for imodel in range(nmodels):
            ...
            for target, result in zip(problem.targets, results):
                if isinstance(result, gf.SeismosizerError) or not isinstance(
                ...
                if target.misfit_config.domain == "cc_max_norm":
                    ...
                    for tr_filt, tr_proc, tshift in (
                        (result.filtered_obs, result.processed_obs, 0.0),
                        (result.filtered_syn, result.processed_syn, result.tshift),
                    ):
        ...
        for cg in cgs:
            ...
            frame_to_target, nx, ny, nxx, nyy = layout(source, targets, nxmax, nymax)
            ...
            for iy in range(ny):
                for ix in range(nx):
                    ...
                    if (iyy, ixx) not in figures:
                        ...
                        figures[iyy, ixx] = (item, plt.figure(figsize=self.size_inch))
                    ...
                    amin, amax = trace_minmaxs[target.normalisation_family, target.path]
                    ...
                    if target.misfit_config.domain == "cc_max_norm":
                        axes.set_ylim(-10.0 * space_factor, 10.0)
                    else:
                    ...
                    for imodel, result in enumerate(target_to_results[target]):
                        ...
                        plot_dtrace(
                            axes2, dtrace, space, 0.0, 1.0, fc="none", ec=syn_color
                        )
                        ...
                        for tmark, text, ha in [
                            ...
                            (tmarks[1], "$\\Delta$ " + meta.str_duration(dur), "right"),
                        ]:
                            axes2.annotate(
                                text,
                                xy=(tmark, -0.9),
...
""",
    ...
    def draw_figures(self, ds, history):
        ...
        gcms = problem.combine_misfits(misfits[:1, :, :], get_contributions=True)[0, :]
            ...
            for iy in range(ny):
                for ix in range(nx):
                    ...
                    if target.misfit_config.domain == "cc_max_norm":
                        ...
                        asmax = amp_spec_maxs[target.normalisation_family, target.path]
                    ...
                    plot_trace(axes, result.processed_syn, color=syn_color, lw=1.0)

                    plot_trace(axes, result.processed_obs, color=obs_color, lw=0.75)
...
""",
    ...
    def draw_figures(self, problem, dataset, history):
        ...
        if history:
            ...
            gms = gms[isort]
            misfits = history.misfits[isort, :]
            gcms = problem.combine_misfits(misfits[:1, :, :], get_contributions=True)[
                0, :
            ]
        ...
        for cg in cgs:
            ...
            assert all(
                target_index[target][0] == target_index[target][1] - 1
                for target in targets
            )

            itargets = num.array([target_index[target][0] for target in targets])
            ...
            if history:
                item = PlotItem(
                    ...
                    title="Station misfit contributions (%s)" % cg_str,
                    description="\n\nMarkers are scaled according to their "
                    "misfit contribution for the globally best "
                    "source model.",
                )
                fig, ax, legend = self.plot_station_distribution(
                    azimuths, distances, gcms[itargets], labels
                )
                legend.set_title("Contribution", prop=dict(size=self.font_size))
