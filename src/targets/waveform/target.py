...
from pyrocko.guts import Object, String, Float, Bool, Int, StringChoice, Timestamp, List
...
class WaveformMisfitConfig(MisfitConfig):
    ...
    tmin = gf.Timing.T(
        optional=True, help="Start of main time window used for waveform fitting."
    )
    tmax = gf.Timing.T(
        optional=True, help="End of main time window used for waveform fitting."
    )
    tfade = Float.T(
        optional=True,
    ...
    pick_phasename = String.T(
        optional=True,
        help="Name of picked phase for alignment of observed and synthetic " "traces.",
    )
    domain = DomainChoice.T(
        default="time_domain",
        help="Type of data characteristic to be fitted.\n\nAvailable choices "
        "are: %s" % ", ".join("``'%s'``" % s for s in DomainChoice.choices),
    )
    norm_exponent = Int.T(
        default=2, help="Exponent to use in norm (1: L1-norm, 2: L2-norm)"
    )
...
def log_exclude(target, reason):
    logger.debug("Excluding potential target %s: %s" % (target.string_id(), reason))
...
class WaveformTargetGroup(TargetGroup):
    ...
    distance_min = Float.T(
        optional=True, help="excludes targets nearer to source, along a great circle"
    )
    distance_max = Float.T(
        optional=True, help="excludes targets farther from source, along a great circle"
    )
    distance_3d_min = Float.T(
        optional=True, help="excludes targets nearer from source (direct distance)"
    )
    distance_3d_max = Float.T(
        optional=True, help="excludes targets farther from source (direct distance)"
    )
    depth_min = Float.T(optional=True, help="excludes targets with smaller depths")
    depth_max = Float.T(optional=True, help="excludes targets with larger depths")
    include = List.T(
        String.T(),
        optional=True,
    ...
    channels = List.T(
        String.T(), optional=True, help="set channels to include, e.g. ['Z', 'T']"
    )
            ...
            for st in ds.get_stations():
                for cha in self.channels:
                    ...
                    if (
                        ...
                        and target.distance_to(origin) < self.distance_min
                    ):
                        log_exclude(target, "distance < distance_min")
                        continue

                    if (
                        self.distance_max is not None
                        and target.distance_to(origin) > self.distance_max
                    ):
                        log_exclude(target, "distance > distance_max")
                        continue

                    if (
                        self.distance_3d_min is not None
                        and target.distance_3d_to(origin) < self.distance_3d_min
                    ):
                        log_exclude(target, "distance_3d < distance_3d_min")
                        continue

                    if (
                        self.distance_3d_max is not None
                        and target.distance_3d_to(origin) > self.distance_3d_max
                    ):
                        log_exclude(target, "distance_3d > distance_3d_max")
                        continue

                    if self.depth_min is not None and target.depth < self.depth_min:
                        log_exclude(target, "depth < depth_min")
                        continue

                    if self.depth_max is not None and target.depth > self.depth_max:
...
class WaveformPiggybackSubtarget(Object):
    ...
    def evaluate(self, tr_proc_obs, trspec_proc_obs, tr_proc_syn, trspec_proc_syn):
...
class WaveformMisfitTarget(gf.Target, MisfitTarget):
    ...
    def get_combined_weight(self):
        if self._combined_weight is None:
            ...
            for analyser in self.analyser_results.values():
                w *= analyser.weight
            self._combined_weight = num.array([w], dtype=num.float64)
    ...
    def get_pick_shift(self, engine, source):
        ...
        if config.pick_synthetic_traveltime and config.pick_phasename:
            store = engine.get_store(self.store_id)
            tsyn = source.time + store.t(config.pick_synthetic_traveltime, source, self)
    ...
    def post_process(self, engine, source, tr_syn):
        ...
        tmin_fit, tmax_fit, tfade, tfade_taper = self.get_taper_params(engine, origin)
        ...
        try:
            ...
            mr = misfit(
                ...
                taper=trace.CosTaper(
                    tmin_fit - tfade_taper, tmin_fit, tmax_fit, tmax_fit + tfade_taper
                ),
    ...
    def prepare_modelling(self, engine, source, targets):
        tmin_fit, tmax_fit, tfade, tfade_taper = self.get_taper_params(
            engine, self.get_origin_source(source)
        )
    ...
    def finalize_modelling(self, engine, source, modelling_targets, modelling_results):
...
tautoshift**2 / tautoshift_max**2``
    ...
    if domain in ("time_domain", "envelope", "absolute"):
        ...
        nshift_max = max(0, min(a.size - 1, int(math.floor(tautoshift_max / deltat))))
        ...
        ctr = trace.correlate(
            tr_proc_syn, tr_proc_obs, mode="same", normalization="normal"
        )
    ...
    if result_mode == "full":
        result = WaveformMisfitResult(
            misfits=num.array([[m, n]], dtype=num.float64),
        ...
        result = WaveformMisfitResult(misfits=num.array([[m, n]], dtype=num.float64))
    else:
        assert False
...
def _extend_extract(tr, tmin, tmax):
    ...
    n = tr.data_len()
    a = num.empty(nframe, dtype=num.float64)
...
def _process(tr, tmin, tmax, taper, domain):
    ...
    if domain == "envelope":
        ...
        padded = num.zeros(nfft, dtype=num.float64)
...
def weed(origin, targets, limit, neighborhood=3):
    ...
    deleted, meandists_kept = weeding.weed(
        azimuths, dists, badnesses, nwanted=limit, neighborhood=neighborhood
    )
