...
shape_out_path = "/home/stu207505/master/geoout/"
km = 1e3
d2r = num.pi / 180.0
guts_prefix = "grond"


def surface_projection_faultline(source):
    from pyrocko import orthodrome as od

    fn, fe = source.outline(cs="latlon").T
    # in km
    parallel_offset_surface = source.depth / num.tan(num.deg2rad(source.dip))
    fe_off = -num.cos(num.deg2rad(source.strike)) * parallel_offset_surface
    fn_off = num.sin(num.deg2rad(source.strike)) * parallel_offset_surface

    fe_proj = []
    fn_proj = []
    for es, ns in zip(fe[:2], fn[:2]):
        latlonnew = od.ne_to_latlon(ns, es, fn_off, fe_off)
        fe_proj.append(latlonnew[1])
        fn_proj.append(latlonnew[0])

    fault_line_e = num.array(fe_proj)
    fault_line_n = num.array(fn_proj)
    return fault_line_e, fault_line_n


def export_ensemble_shape(filen, problem, models, target):
    ...
    out_file = filen + "_moment_area.tab"
    f = open(out_file, "w")
    f.write(("#mean_depth[m] slip[m] area[m2] moment_atmu_32GP[Nm]\n"))

    for model in models:
        source = problem.get_source(model)
        fn_sub_lat, fe_sub_lon = source.outline(cs="latlon").T

        fault_line_e, fault_line_n = surface_projection_faultline(source)

        polygon_geom = Polygon(zip(fn_sub_lat, fe_sub_lon))
        polygon = gpd.GeoDataFrame(index=[0], crs=crs, geometry=[polygon_geom])

        polygon_geom_edge = Polygon(
            zip(
                [
                    fe_sub_lon[0],
                    fe_sub_lon[1],
        ...
        polygon_edge = gpd.GeoDataFrame(
            index=[0], crs=crs, geometry=[polygon_geom_edge]
        )

        polygon_geom_edge_proj = Polygon(
            zip(
                [
                    fault_line_e[0],
                    fault_line_e[1],
        ...
        polygon_edge_proj = gpd.GeoDataFrame(
            index=[0], crs=crs, geometry=[polygon_geom_edge_proj]
        )

        polylist.append(polygon)
        edgelist.append(polygon_edge)
        projlist.append(polygon_edge_proj)

        # moment = 10 ** ((source.get_magnitude(store=target.store_id, target=target) +6.03) *3. / 2.)
        x, y, z = source.outline(cs="xyz").T
        mean_depth = (z[1] + z[2]) / 2.0
        moment = source.slip * source.length * source.width * 32.0e9
        area = source.length * source.width
        f.write("%3.4f %3.4f %3.4f %8.8E\n" % (mean_depth, source.slip, area, moment))

    f.close()
    polys = gpd.GeoDataFrame(pd.concat(polylist), crs=crs)
    polys.to_file(filename=filen + ".shp", driver="ESRI Shapefile")
    polys_edge = gpd.GeoDataFrame(pd.concat(edgelist), crs=crs)
    polys_edge.to_file(filename=filen + "_edge" + ".shp", driver="ESRI Shapefile")
    polys_edge_proj = gpd.GeoDataFrame(pd.concat(projlist), crs=crs)
    polys_edge_proj.to_file(
        filename=filen + "_edge_surfproj" + ".shp", driver="ESRI Shapefile"
    )


def export_source_shape(filen, lon_point_list, lat_point_list, source=None):
    import geopandas as gpd
    from shapely.geometry import Polygon
    ...
    # polygon.to_file(filename=filen + 'geojson.gjson', driver='GeoJSON')
    polygon.to_file(filename=filen + ".shp", driver="ESRI Shapefile")


def drape_displacements(
    displacement, shad_data, mappable, shad_lim=(0.4, 0.98), contrast=1.0, mask=None
):
...
def scale_axes(axis, scale, offset=0.0, suffix=""):
    from matplotlib.ticker import ScalarFormatter

    class FormatScaled(ScalarFormatter):
        @staticmethod
        def __call__(value, pos):
            return "{:,.2f}{:}".format((offset + value) * scale, suffix).replace(
                ",", " "
            )
...
class SatelliteTargetDisplacement(PlotConfig):
    ...
    size_cm = Tuple.T(2, Float.T(), default=(22.0, 12.0))
    colormap = String.T(default="RdBu", help="Colormap for the surface displacements")
    ...
    fit = StringChoice.T(
        default="best",
        choices=["best", "mean"],
        help="Show the 'best' or 'mean' fits and source model from the" " ensamble.",
    )

    show_topo = Bool.T(default=True, help="Drape displacements over the topography.")
    ...
    show_leaf_centres = Bool.T(
        default=False, help="show the center points of Quadtree leaves"
    )
    ...
    nticks_x = Int.T(optional=True, help="Number of ticks on the x-axis.")
...
""",
    ...
    def draw_static_fits(self, ds, history, optimiser, closeup=False):
        ...
        # logger.warning(problem.parameter_names)
        ...
        if self.fit == "best":
            ...
            gms = gms[isort]
            # print('gms 0:',gms[isort[0]])
            # print('gms -1:',gms[isort[-1]])
            models2 = models[isort, :]
            xbest = models2[0, :]
        ...
        def init_axes(ax, scene, title, last_axes=False):
            ...
            if scene.frame.isMeter():
                ...
                scale_x = dict(scale=1.0 / km)
                scale_y = dict(scale=1.0 / km)
                utm_E, utm_N, utm_zone, utm_zone_letter = utm.from_latlon(
                ...
                scale_x = dict(scale=1.0, suffix="°")
                scale_y = dict(scale=1.0, suffix="°")
                scale_x["offset"] = source.effective_lon
                scale_y["offset"] = source.effective_lat

                ax.set_aspect(1.0 / num.cos(source.effective_lat * d2r))
        ...
        def draw_source(ax, scene, source, i=0):
            if isinstance(source, gf.CombiSource):
                for sources in source.subsources:
                    draw_source(ax, scene, sources, i)
                    i += 1
            else:
                if (
                    not scene.meta.time_primary
                    < source.time
                    < scene.meta.time_secondary
                ):
                    pass
                else:
                    if scene.frame.isMeter():
                        ...
                        fn, fe = source.outline(cs="latlon").T
                        print(
                            "center fault lon, lat:", num.mean(fe[:4]), num.mean(fn[:4])
                        )
                        if isinstance(source, gf.RectangularSource):
                            fault_line_e, fault_line_n = surface_projection_faultline(
                                source
                            )

                            # export_ensemble_shape(
                            #     shape_out_path + problem.name + "_ensemble_sources",
                            #     problem,
                            #     models,
                            #     sat_targets[0],
                            # )
                            ...
                            # if len(fn)>1:
                            export_source_shape(
                                shape_out_path + problem.name + "_best_source" + str(i),
                                fe,
                                fn,
                                source,
                            )
                            ...
                            export_source_shape(
                                ...
                                + "_best_source_edge"
                                + str(i),
                                [fe[0], fe[1], fe[1], fe[0], fe[0]],
                                [fn[0], fn[1], fn[1], fn[0], fn[0]],
                            )

                        fn -= problem.get_source(model).effective_lat
                        fe -= problem.get_source(model).effective_lon
                        if isinstance(source, gf.RectangularSource):
                            fault_line_n -= problem.get_source(model).effective_lat
                            fault_line_e -= problem.get_source(model).effective_lon
                    # source is centered
                    ax.scatter(0.0, 0.0, color="black", s=3, alpha=0.5, marker="o")
                    ax.fill(
                        fe,
                        fn,
        ...
        def get_displacement_rgba(displacements, scene, mappable):
            ...
            # print(self.common_color_scale)
            if self.common_color_scale and not self.displacement_unit == "rad":
                abs_displ = num.abs(displacements).max()
                mappable.set_clim(-0.002, 0.002)
                # print(abs_displ)

            if self.show_topo:
                try:
                    elevation = scene.get_elevation(interpolation="bivariate")
            ...
            rgb_arr[num.isnan(arr)] = 1.0
            rgb_arr[scene.displacement_mask] = 1.0

            return rgb_arr

        def draw_leaves(ax, scene, offset_e=0.0, offset_n=0.0):
        ...
        def generate_plot(sat_target, result, ifig):
            ...
            item = PlotItem(
                ...
                title="Satellite Surface Displacements - %s - %s"
                % (
                    datetime.fromtimestamp(scene.meta.time_primary).strftime(
                        "%d. %B %Y %I:%M%p"
                    ),
                    datetime.fromtimestamp(scene.meta.time_secondary).strftime(
                        "%d. %B %Y %I:%M%p"
                    ),
                ),
                description="""
...
""",
            ...
            if self.displacement_unit == "rad":
                ...
                if wavelength is None:
                    raise AttributeError("The satellite's wavelength is not set")
            ...
            ax = axes[2]
            ax.imshow(
                get_displacement_rgba(res, scene, cmw), extent=im_extent, origin="lower"
            )
            ...
            cbar_args = dict(
                orientation="horizontal", format=FuncFormatter(cfmt), use_gridspec=True
            )
...
class SatelliteTargetDisplacementCloseup(SatelliteTargetDisplacement):
    ...
    map_scale = Float.T(
        default=2.0, help="Scale the map surroundings, larger value zooms out."
    )
