...
class SatelliteMisfitConfig(MisfitConfig):
    ...
    optimise_orbital_ramp = Bool.T(
        default=True, help="Switch to account for a linear orbital ramp or not"
    )
    ranges = Dict.T(
        String.T(),
        gf.Range.T(),
...
class SatelliteTargetGroup(TargetGroup):
    ...
    def get_targets(self, ds, event, default_path="none"):
        ...
        for scene in ds.get_kite_scenes():
            # logger.warning(ds.events[0].name)
            if (
                scene.meta.scene_id not in self.kite_scenes
                and "*all" not in self.kite_scenes
            ):
                continue
            for ev in ds.events:
                if (
                    scene.meta.time_primary >= ev.time
                    or scene.meta.time_secondary <= ev.time
                ):
                    continue
            logger.warning("YEP")
            qt = scene.quadtree
            ...
            if qt.frame.isDegree():
                logger.debug(
                    'Target "%s" is referenced in degree.' % scene.meta.scene_id
                )
                ...
                logger.debug(
                    'Target "%s" is referenced in meter.' % scene.meta.scene_id
                )
...
class SatelliteMisfitResult(gf.Result, MisfitResult):
    ...
    statics_syn = Dict.T(
        optional=True, help="Predicted static displacements for a target (synthetics)."
    )
    statics_obs = Dict.T(
        optional=True, help="Observed static displacement for a target."
    )
...
class SatelliteMisfitTarget(gf.SatelliteTarget, MisfitTarget):
    ...
    def post_process(self, engine, source, statics):
        ...
        if self.misfit_config.optimise_orbital_ramp:
            ...
            stat_level += (
                quadtree.leaf_center_distance[:, 0] * self.parameter_values["ramp_east"]
            )
            stat_level += (
                quadtree.leaf_center_distance[:, 1]
    ...
    def init_bootstrap_residuals(self, nbootstraps, rstate=None):
        ...
        for ibs in range(nbootstraps):
            if not (ibs + 1) % 5:
                logger.info(
                    "Calculating noise realisation %d/%d." % (ibs + 1, nbootstraps)
                )
