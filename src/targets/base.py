...
class TargetGroup(Object):
    normalisation_family = gf.StringID.T(
        optional=True, help="Group with common misfit normalisation"
    )
    path = gf.StringID.T(
        optional=True, help="Targets.id will be prefixed with this path"
    )
    weight = Float.T(default=1.0, help="Additional manual weight of the target group")
    interpolation = gf.InterpolationMethod.T(
        default="nearest_neighbor", help="Interpolation from pre-calculated GF store."
    )
    store_id = gf.StringID.T(
        optional=True, help="ID of the Green's function store for this TargetGroup."
    )
...
class MisfitResult(Object):
    misfits = Array.T(shape=(None, 2), dtype=num.float64)
...
class MisfitTarget(Object):
    ...
    analyser_results = Dict.T(
        gf.StringID.T(), AnalyserResult.T(), help="Dictionary of analyser results"
    )
    normalisation_family = gf.StringID.T(
    ...
    misfit_config = MisfitConfig.T(
        default=MisfitConfig.D(), help="Misfit configuration"
    )
    bootstrap_weights = Array.T(dtype=num.float64, serialize_as="base64", optional=True)
    bootstrap_residuals = Array.T(
        dtype=num.float64, serialize_as="base64", optional=True
    )
    ...
    def misfits_string_ids(self):
        raise NotImplementedError(
            "%s does not implement misfits_string_id" % self.__class__.__name__
        )
    ...
    def get_combined_weight(self):
        if self._combined_weight is None:
            self._combined_weight = num.ones(1, dtype=num.float64)
