...
class GNSSTargetMisfitPlot(PlotConfig):
    ...
    size_cm = Tuple.T(
        2, Float.T(), default=(30.0, 30.0), help="width and length of the figure in cm"
    )
    ...
    radius = Float.T(
        optional=True, help="radius of the map around campaign center lat/lon"
    )
...
""",
    ...
    def draw_gnss_fits(self, ds, history, optimiser, vertical=False):
        ...
        results = problem.evaluate(xbest, result_mode="full", targets=gnss_targets)
        ...
        def plot_gnss(gnss_target, result, ifig, vertical=False):
            ...
            item = PlotItem(
                ...
                title="Static GNSS Surface Displacements - Campaign %s" % campaign.name,
                description="""
...
"""
            ...
            if self.radius is None:
                ...
                radius = od.distance_accurate50m_numpy(
                    lat[num.newaxis], lon[num.newaxis], coords[:, 0].max(), coords[:, 1]
                ).max()
                radius *= 1.1
            ...
            model_camp = gnss.GNSSCampaign(
                stations=copy.deepcopy(campaign.stations), name="grond model"
            )
            ...
            if isinstance(problem, CMTProblem):
                ...
                xx = num.trace(mt) / 3.0
                mc = num.matrix([[xx, 0.0, 0.0], [0.0, xx, 0.0], [0.0, 0.0, xx]])
                mc = mt - mc
                mc = (
                    mc
                    / event.moment_tensor.scalar_moment()
...
class GNSSStationDistribution(StationDistributionPlot):
    ...
    def make(self, environ):
        ...
        cm.create_group_mpl(
            ...
            title="GNSS Station Distribution",
            section="checks",
            feather_icon="target",
            description="""
    ...
    def draw_figures(self, problem, dataset, history):
        ...
        for target in targets:
            ...
            fig, ax, legend = self.plot_station_distribution(
                azimuths, distances, ws_n, labels, legend_title="Weight, N components"
            )
            ...
            fig, ax, legend = self.plot_station_distribution(
                azimuths, distances, ws_e, labels, legend_title="Weight, E components"
            )
            ...
            fig, ax, legend = self.plot_station_distribution(
                azimuths, distances, ws_u, labels, legend_title="Weight, U components"
            )
...
def get_plot_classes():
    return [GNSSTargetMisfitPlot, GNSSStationDistribution]
