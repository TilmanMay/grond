...
class GNSSCampaignMisfitResult(MisfitResult):
    ...
    statics_syn = Dict.T(optional=True, help="Synthetic gnss surface displacements")
    statics_obs = Dict.T(optional=True, help="Observed gnss surface displacements")
...
class GNSSCampaignTargetGroup(TargetGroup):
    ...
    def get_targets(self, ds, event, default_path="none"):
        ...
        for camp in ds.get_gnss_campaigns():
            if (
                camp.name not in self.gnss_campaigns
                and "*all" not in self.gnss_campaigns
            ):
                continue

            if not isinstance(self.misfit_config, GNSSCampaignMisfitConfig):
                raise AttributeError(
                    "misfit_config must be of type" " GNSSCampaignMisfitConfig."
                )
...
class GNSSCampaignMisfitTarget(gf.GNSSCampaignTarget, MisfitTarget):
    ...
    def misfits_string_ids(self):
        return [
            "%s.%s" % (self.path, station.code) for station in self.campaign.stations
        ]
    ...
    def weights(self):
        ...
        if self._weights is None:
            ...
            if not num.any(covar.diagonal()):
                logger.warning(
                    "GNSS Stations have an empty covariance matrix."
                    " Weights will be all equal."
                )
                num.fill_diagonal(covar, 1.0)
    ...
    def init_bootstrap_residuals(self, nbootstraps, rstate=None):
        logger.info(
            "GNSS campaign %s, bootstrapping residuals"
        ...
        if not num.all(sigmas):
            logger.warning(
                "Bootstrapping GNSS stations is meaningless,"
        ...
        for ibs in range(nbootstraps):
            ...
            syn_noise = syn_noise.reshape(campaign.nstations, 3).sum(axis=1)
