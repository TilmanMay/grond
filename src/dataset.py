...
from pyrocko import util, pile, model, config, trace, marker as pmarker
from pyrocko.io.io_common import FileLoadError
from pyrocko.fdsn import enhanced_sacpz, station as fs
from pyrocko.guts import Object, Tuple, String, Float, List, Bool, dump_all, load_all
...
class EventGroup(Object):
    ...
    def from_single_event(self, event):
        eg = EventGroup(name=event.name, event_names=[event.name])
...
def check_events(events, fn):
    for ev in events:
        if not ev.name:
            raise DatasetError('An event in file "%s" has no name.' % fn)

        if ev.lat is None or ev.lon is None:
            logger.warning('Event "%s" from file "%s" has no lat/lon.' % (ev.name, fn))

        if ev.depth is None:
            logger.warning('Event "%s" from file "%s" has no depth.' % (ev.name, fn))

        if ev.time is None:
            logger.warning('Event "%s" from file "%s" has no time.' % (ev.name, fn))
...
class Dataset(object):
    ...
    def add_events(self, events=None, filename=None):
        ...
        if filename is not None:
            ...
            try:
                ...
                logger.warning('Could not load events from file "%s"', filename)
    ...
    def add_waveforms(
        self, paths, regex=None, fileformat="detect", show_progress=False
    ):
        cachedirname = config.config().cache_dir

        logger.debug("Selecting waveform files %s..." % quote_paths(paths))
        fns = util.select_files(paths, regex=regex, show_progress=show_progress)
    ...
    def add_gnss_campaigns(self, paths):
        paths = util.select_files(paths, regex=r"\.yml|\.yaml", show_progress=False)
    ...
    def is_blacklisted(self, obj):
        ...
        return (
            nsl in self.blacklist
            or nsl[1:2] in self.blacklist
            or nsl[:2] in self.blacklist
        )
    ...
    def is_whitelisted(self, obj):
        ...
        if (
            nsl in self.whitelist
            or nsl[1:2] in self.whitelist
            or nsl[:2] in self.whitelist
        ):
    ...
    def get_tmin_tmax(self, obj):
        if isinstance(obj, trace.Trace):
            ...
            raise InvalidObject(
                'Cannot get tmin and tmax from given object of type "%s"' % type(obj)
            )
    ...
    def get_response(self, obj, quantity="displacement"):
        if (self.responses is None or len(self.responses) == 0) and (
            self.responses_stationxml is None or len(self.responses_stationxml) == 0
        ):
        ...
        keys_x = [(net, sta, loc, cha), (net, sta, "", cha), ("", sta, "", cha)]
                        ...
                        if quantity == "displacement":
                            ...
                            candidates.append(
                                trace.MultiplyResponse(
                                    [x.response, trace.DifferentiationResponse()]
                                )
                            )
                        elif quantity == "acceleration":
                            candidates.append(
                                trace.MultiplyResponse(
                                    [x.response, trace.DifferentiationResponse(2)]
                                )
                            )
                        else:
                            assert False
    ...
    def get_waveform_raw(
        ...
        if self.clip_handling == "by_nsl":
            ...
            if self.has_clipping((net, sta, loc, cha), tmin, tmax):
                raise NotFound("Waveform clipped:", (net, sta, loc, cha))
    ...
    def get_waveform_restituted(
        ...
        for tr in trs_raw:
            ...
            try:
                ...
                raise NotFound(
                    "Instrument response deconvolution failed " "(divide by zero)",
                    tr.nslc_id,
                )

        return trs_restituted, trs_raw

    def _get_projections(self, station, backazimuth, source, target, tmin, tmax):
        ...
        projections = []
        projections.extend(
            station.guess_projections_to_enu(out_channels=("E", "N", "Z"))
        )
        ...
        if backazimuth is not None:
            projections.extend(
                station.guess_projections_to_rtu(
    ...
    def _get_waveform(
        ...
        cache_k = nslc + (tmin, tmax, tuple(freqlimits), tfade, deltat, tpad, quantity)
        ...
        if syn_test:
            if not syn_test.respect_data_availability:
                ...
                tr = syn_test.get_waveform(
                    nslc, tmin, tmax, tfade=tfade, freqlimits=freqlimits
                )
        ...
        try:
            ...
            for matrix, in_channels, out_channels in projections:
                deps = trace.project_dependencies(matrix, in_channels, out_channels)
                ...
                try:
                    ...
                    if channel in deps:
                        ...
                        trs_projected.extend(
                            trace.project(
                                trs_restituted_group, matrix, in_channels, out_channels
                            )
                        )
                ...
                for tr in trs_projected:
                    if tr.channel == channel:
                        tr_syn = syn_test.get_waveform(
                            tr.nslc_id, tmin, tmax, tfade=tfade, freqlimits=freqlimits
                        )
                        ...
                        if tr_syn:
                            if syn_test.real_noise_scale != 0.0:
                                ...
                                tr_noise = tr.copy()
                                tr_noise.set_ydata(
                                    tr_noise.get_ydata() * syn_test.real_noise_scale
                                )
            ...
            if debug:
                ...
                raise NotFound("waveform not available", station.nsl() + (channel,))
        ...
        if self._picks is None:
            ...
            for marker in self.pick_markers:
                if isinstance(marker, pmarker.EventMarker):
                    ...
                    if name in names:
                        raise DatasetError('Duplicate event name "%s" in picks.' % name)
            ...
            for marker in self.pick_markers:
                if isinstance(marker, pmarker.PhaseMarker):
                    ...
                    if ehash is None or ehash not in hash_to_name:
                        raise DatasetError(
                            "Unassociated pick: %s.%s.%s, %s" % (nsl + (phasename,))
                        )
                    ...
                    if (nsl, phasename, eventname) in picks:
                        raise DatasetError(
                            "Duplicate pick: %s.%s.%s, %s" % (nsl + (phasename,))
                        )
...
class DatasetConfig(HasPaths):
    ...
    events_path = Path.T(
        help="File with hypocenter information and possibly" " reference solutions"
    )
    event_groups_path = Path.T(
        optional=True, help="File defining event groups for multi-source inversions"
    )
    waveform_paths = List.T(
        Path.T(), optional=True, help="List of directories with raw waveform data"
    )
    ...
    apply_displaced_sampling_workaround = Bool.T(
        optional=True, default=False, help="Work around displaced sampling issues."
    )
    extend_incomplete = Bool.T(default=False, help="Extend incomplete seismic traces.")
    ...
    def get_dataset(self, event_or_group_name):
        if event_or_group_name not in self._ds:
            ...
            def extra(path):
                return expand_template(path, dict(event_name=event_or_group_name))
